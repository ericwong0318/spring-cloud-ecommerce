...
**What to build:** Testcontainers suite: (1) Full flow: Order → reserve → auth → capture → confirm → partial ship → deliver; (2) Idempotency — duplicate `OrderEvent.CREATED` processed once.
...
- [ ] Create `system-test` module Testcontainers configuration: PostgreSQL (per service or shared), Kafka/RabbitMQ
- [ ] Implement test infrastructure:
  - `TestcontainersConfig` with `@Container` for PostgreSQL, Kafka
